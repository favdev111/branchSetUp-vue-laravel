...
function analyzeAddress() {
    ...
    let $addressList = $($addressTab).find('.address-row.view');
...
function analyzePhone() {
    let $tabs = $('.tab');
    let $phoneTab = $tabs[2];
    let $list = $($phoneTab).find('.phone-row.view');
...
function analyzeEmail() {
    let $tabs = $('.tab');
    let $emailTab = $tabs[3];
    let $list = $($emailTab).find('.email-row.view');
    $list.each(function (e) {
        let $current = $($list[e])
        var new_email = {
            "type" : "",
...
function analyzeTax() {
    let $tabs = $('.tab');
    let $taxTab = $tabs[4];
    let $list = $($taxTab).find('.tax-row.view');
    $list.each(function (e) {
        let $current = $($list[e])
        var new_tax = {
            "type" : "",
...
function analyzeParameters() {
    ...
    $($tabs[parametersTab]).find(".parameters-input").each(function(){
        if($(this).attr('type') == 'checkbox')
            $("#parameters-col").find('[name="'+$(this).attr('name')+'"]').prop('checked', $(this).prop('checked'));
        else
            $("#parameters-col").find('[name="'+$(this).attr('name')+'"]').val($(this).val());
    });
}

function analyzeZipcodes() {
    window.zipcodes = [];
    let $tabs = $('.tab');
    $($tabs[zipcodesTab]).find(".zipcode-input").each(function(){
        if($(this).prop('checked'))
            zipcodes.push( $(this).val() );
    });
    $("#zipcodes-report").text( zipcodes.join(', ') );
    console.log(zipcodes);
}
$(".zipcode-input").change(analyzeZipcodes);

// HERE-WE-CODE-JS
//Output Provider
function analyzeProvider() {
    let $tabs = $('.tab');
    let $providerTab = $tabs[providerTab];
    ...
    var parameters = {};
    document.querySelectorAll('.parameter-active-checkbox').forEach(function (evt) {
        if (evt.target.checked) {
            parameters[evt.target.id] = {
                id: evt.target.id,
                status: 'Inactive'
            }
        } else {
...
function cleanReport() {
    ...
    let $supplierDiv = $("#supplier")[0];
    $($supplierDiv).find('.supplier-row[style*="display: block;"]').remove();
...
function makeValidationAddress()
{
    ...
    $(".parameters-input").each(function(){
        $('#registerForm').bootstrapValidator('addField', $(this));
    });
}
...
function onSubmitBtn() {
    ...
    br.type = $($branchDiv).find('input[name="br_type"]').val();
    ...
    var supplier_arr = [];
    $supplierDiv =$($curTab).find('#supplier');
    ...
    entry.type = $($supplierDiv).find('select[name="sp_type[]"]').val();
    entry.name = $($supplierDiv).find('select[name="sp_name[]"]').val();

    supplier_arr.push(entry);
    
    updatedIds    =   $("#updatedIds").val();
    
    let parameters = {};
    $("#parameters-col .parameters-input").each(function(){
...
function nextPrev(number) {
    ...
    if (number == 1 && !validateForm(currentTab)) 
        return false;
...
function validateForm(number) {
    let $tabs = $('.tab');
    let $curTab = $tabs[number];
    let $cnt = $($curTab).find(".view").length;
	if($cnt < 1)
    {
		if(!validateTab(number))
		{
			return false;
		} 
		switch (number) {
...
function onDeleteAddress(){
    ...
    let $last_row = $($address_tab).find(".address-row.view").last();
    let $cnt = $($address_tab).find(".address-row.view").length;
    if($cnt <= 1)
    {
...
function onDeletePhone(){
    let $tabs = $('.tab');
    let $curTab = $tabs[2];
    let $last_row = $($curTab).find(".phone-row.view").last();
    let $cnt = $($curTab).find(".phone-row.view").length;
    if($cnt <= 1)
    {
        alert('You need at least one!');
        return;
...
function onDeleteEmail(){
    let $tabs = $('.tab');
    let $curTab = $tabs[3];
    let $last_row = $($curTab).find(".email-row.view").last();
    let $cnt = $($curTab).find(".email-row.view").length;
    if($cnt <= 1)
    {
        alert('You need at least one!');
        return;
...
function onDeleteTax(){
    let $tabs = $('.tab');
    let $curTab = $tabs[4];
    let $last_row = $($curTab).find(".tax-row.view").last();
    let $cnt = $($curTab).find(".tax-row.view").length;
    if($cnt <= 1)
    {
        alert('You need at least one!');
        return;
...
$(function() {
    $(document).on('keyup', '[name^=address_zip]', function( e ) {
        let zipCode      = $(this).val();
        let countryInput = $(this).closest('.address-row').find('[name^=address_county]');
		let cityInput = $(this).closest('.address-row').find('[name^=address_city]');
        fillCountryInput(zipCode, countryInput, cityInput);
    });

    $(document).on('keyup', '[name^=add_zip]', function( e ) {
        let zipCode      = $(this).val();
        let countryInput = $(this).closest('.address-row').find('[name=add_county]');
		let cityInput = $(this).closest('.address-row').find('[name=add_city]');
        fillCountryInput(zipCode, countryInput, cityInput);
    });
});

function fillCountryInput(zipCode, countryInput, cityInput)
{
    if(zipCode.length!= 5){
        return;
    }
            ...
            /*for (var component in results[0]['address_components']) {
                for (var i in results[0]['address_components'][component]['types']) {
                    if (results[0]['address_components'][component]['types'][i] == "administrative_area_level_2") {
                        countryName = results[0]['address_components'][component]['long_name'];
                        countryName = countryName.replace(' County', '');
                    }
					if (results[0]['address_components'][component]['types'][i] == "locality" || results[0]['address_components'][component]['types'][i] == "political") {
                        cityName = results[0]['address_components'][component]['long_name'];
